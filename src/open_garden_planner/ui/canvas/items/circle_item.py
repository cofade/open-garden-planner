...
class CircleItem(RotationHandleMixin, ResizeHandlesMixin, GardenItemMixin, QGraphicsEllipseItem):
    ...
    def contextMenuEvent(self, event: QGraphicsSceneContextMenuEvent) -> None:
        ...
        if is_bed_type(self.object_type):
            ...
            plant_harvest_action = menu.addAction(
        ...
        elif action == plant_harvest_action and plant_harvest_action is not None:
            scene = self.scene()
            if scene:
                views = scene.views()
                if views and hasattr(views[0], "request_log_harvest"):
                    views[0].request_log_harvest(str(self.item_id), self.name)
