"""Harvest log dialog (US-C1, issue #188).

Right-click a plant (or a bed whose plants are children) → "Log Harvest…" to
open this dialog. On accept, ``result_entry()`` returns a populated
``HarvestEntry`` ready to be wrapped in an ``AddHarvestEntryCommand``.

Mirrors ``pest_log_dialog.py`` (US-12.7): two tabs (collapsed to one in edit
mode):
  * Entry   — date, quantity, unit, quality, notes, optional photo.
  * History — past entries for this target with per-year totals and
...
# Common units offered in the editable unit combo. The combo is editable so a
# user can type any unit (e.g. "litres"); these are just quick picks.
_DEFAULT_UNITS = ["g", "kg", "pcs", "bunches"]


class HarvestLogDialog(QDialog):
    """Modal dialog for entering a single harvest entry."""
    ...
    def __init__(
        ...
        target_id: str = "",
        target_name: str = "",
        existing_history: HarvestLogHistory | None = None,
        edit_mode: bool = False,
        edit_entry: HarvestEntry | None = None,
        ...
        self._edit_entry_id = edit_entry.id if (edit_mode and edit_entry) else None
        self._edit_journal_note_id = (
            edit_entry.journal_note_id if (edit_mode and edit_entry) else None
        )
        self._project_manager = project_manager
        self._command_manager = command_manager
        self._existing_history = existing_history
        # Stored as project-relative path (POSIX separators) once attached.
        self._photo_path: str | None = (
            edit_entry.photo_path if edit_entry is not None else None
        )
        ...
        if edit_entry is not None:
            self._populate(edit_entry)

    # ── UI construction ──────────────────────────────────────────────────────

    def _setup_ui(self, existing_history: HarvestLogHistory | None) -> None:
    ...
    def _build_entry_tab(self) -> QWidget:
        ...
        # Unit (editable so users can type any unit per crop)
        self._unit_combo = QComboBox()
        self._unit_combo.setEditable(True)
        self._unit_combo.addItems(_DEFAULT_UNITS)
        self._unit_combo.setCurrentText("g")
        form.addRow(self.tr("Unit"), self._unit_combo)

        # Quality note
        self._quality_edit = QLineEdit()
        self._quality_edit.setPlaceholderText(self.tr("e.g. excellent, slightly bitter"))
    ...
    def _build_history_tab(self, history: HarvestLogHistory | None) -> QWidget:
        scroll = QScrollArea()
        ...
        layout.addWidget(QLabel(self.tr("Totals by year")))
        self._totals_container = QWidget()
        self._totals_layout = QVBoxLayout(self._totals_container)
        self._totals_layout.setContentsMargins(0, 0, 0, 0)
        self._totals_layout.setSpacing(2)
        layout.addWidget(self._totals_container)
    ...
    def _refresh_history_view(self, history: HarvestLogHistory | None) -> None:
        for container_layout in (self._history_records_layout, self._totals_layout):
        ...
        entries = list(history.entries) if history is not None else []
        ...
        if not entries:
            ...
            for entry in entries:
                self._history_records_layout.addWidget(self._build_history_row(entry))

        # Per-year totals, grouped by unit so incompatible units never sum.
        ...
        if not totals:
            self._totals_layout.addWidget(QLabel(self.tr("—")))
        else:
            for (year, unit), total in sorted(totals.items()):
                self._totals_layout.addWidget(
                    QLabel(
                        self.tr("{year}: {total:g} {unit}").format(
                            year=year, total=total, unit=unit
                        )
                    )
                )
    ...
    def _build_history_row(self, entry: HarvestEntry) -> QWidget:
        row = QWidget()
        h = QHBoxLayout(row)
        h.setContentsMargins(0, 0, 0, 0)
        h.setSpacing(4)

        h.addWidget(QLabel(self._format_history_row(entry)), 1)

        if self._project_manager is not None and self._command_manager is not None:
            edit_btn = QPushButton(self.tr("Edit"))
            edit_btn.setFixedWidth(60)
            edit_btn.clicked.connect(lambda _, e=entry: self._on_edit_entry(e))
            h.addWidget(edit_btn)

            del_btn = QPushButton(self.tr("Delete"))
            del_btn.setFixedWidth(60)
            del_btn.clicked.connect(lambda _, e=entry: self._on_delete_entry(e))
            h.addWidget(del_btn)

        return row

    def _format_history_row(self, entry: HarvestEntry) -> str:
        text = self.tr("{date} — {qty:g} {unit}").format(
            date=entry.date or "?", qty=entry.quantity, unit=entry.unit
        )
        if entry.quality:
            text = text + self.tr(" ({quality})").format(quality=entry.quality)
        return text

    def _on_edit_entry(self, entry: HarvestEntry) -> None:
        ...
        if sub.exec() != QDialog.DialogCode.Accepted:
            return
        from open_garden_planner.core import EditHarvestEntryCommand  # noqa: PLC0415

        new_entry = sub.result_entry()
        cmd = EditHarvestEntryCommand(self._project_manager, self._target_id, new_entry)
        self._command_manager.execute(cmd)
        self._reload_history_after_change()

    def _on_delete_entry(self, entry: HarvestEntry) -> None:
        ...
        if reply != QMessageBox.StandardButton.Yes:
            return
        from open_garden_planner.core import DeleteHarvestEntryCommand  # noqa: PLC0415

        cmd = DeleteHarvestEntryCommand(self._project_manager, self._target_id, entry.id)
        self._command_manager.execute(cmd)
        self._reload_history_after_change()
    ...
    def _populate(self, entry: HarvestEntry) -> None:
        if entry.date:
            try:
                yyyy, mm, dd = (int(p) for p in entry.date.split("-"))
                self._date_edit.setDate(QDate(yyyy, mm, dd))
            except (ValueError, TypeError):
                pass
        self._quantity_spin.setValue(entry.quantity)
        self._unit_combo.setCurrentText(entry.unit)
        self._quality_edit.setText(entry.quality)
        if entry.notes:
            self._notes_edit.setPlainText(entry.notes)
        self._photo_path = entry.photo_path
        self._update_photo_thumbnail()

    def _on_accept(self) -> None:
        has_qty = self._quantity_spin.value() > 0.0
        on_entry_tab = not hasattr(self, "_tabs") or self._tabs.currentIndex() == 0
        # Zero-quantity warning only when Entry tab is active.
    ...
    def has_new_entry(self) -> bool:
        """True when a new entry should be added on accept.

        Checks quantity content rather than active tab so filling the Entry tab,
        switching to History, then clicking OK still persists the entry.
        """
        if self._edit_mode:
            return False
        if getattr(self, "_force_new_entry", False):
            return True
        return self._quantity_spin.value() > 0.0

    # ── Public API ───────────────────────────────────────────────────────────

    def result_entry(self) -> HarvestEntry:
        qd = self._date_edit.date()
        date_iso = f"{qd.year():04d}-{qd.month():02d}-{qd.day():02d}"
        kwargs: dict[str, Any] = {
            "date": date_iso,
            "quantity": self._quantity_spin.value(),
            "unit": self._unit_combo.currentText().strip() or "g",
            "quality": self._quality_edit.text().strip(),
            "notes": self._notes_edit.toPlainText().strip(),
            "photo_path": self._photo_path,
        }
        if self._edit_entry_id is not None:
            kwargs["id"] = self._edit_entry_id
            kwargs["journal_note_id"] = self._edit_journal_note_id
        return HarvestEntry(**kwargs)
