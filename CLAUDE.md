...
- Phases 1–12 are complete; Phase 13 agent integration is complete through **D2.6** (the D2 tool layer shipped in v1.27.11 via PR #361), and Phases 14 and 15 (Visual Refresh, Packages 1–3) are complete.
- Remaining work is tracked in `docs/roadmap.md`: D2.6 (#330) shipped in v1.27.11 via PR #361, adding `get_geometry`, `set_object_position`, and polygon/polyline `set_vertex`/`add_vertex`/`delete_vertex`; constrained geometry writes permanently refuse while the read exposes the blocking constraints. Next are the four D3 domain stories (#319, #331–#333), plus the permapeople-research Packages F/G (F = #312–#316, G = #311, #317, #318, #320; note #319 is D3 and #321 is standalone, despite the contiguous numbering). D2.4 (#328) and D2.5 (#329) shipped in v1.27.8 via PR #352: UUID-addressed layer tools with locked-layer protection, loader-backed shape/structure creation, HOUSE+ridge composite undo, discovery, and roster drift guards. Phase 15 Package 3 shipped 2026-08-18 as v1.26.8 (3a, PR #322) / v1.26.9 (3b, PR #323) / v1.27.0 (3c, PR #325; ADR-042, FR-30, §8.23–§8.24, §8.21.5). D2.2 + D2.3 shipped together (#326 + #327, PR #335): `resize_object`/`rotate_object`/`set_species`/`set_parent_bed`, plus `ui/canvas/geometry_apply.py` — **the one apply path for every rect-backed resize; do not add another closure** (§8.19, §11.4). Bug #337 (compare-overlay `RuntimeError` on File → New Plan) shipped 2026-08-21 as v1.27.2 (PR #339): `CanvasScene.clear()` is now the one chokepoint that drops every private item-tracking list (`_compare_items`, `_calibration_markers`, and their sibling state) before the C++ teardown — see §11.4. Issue #338 (per-object stacking order, ADR-043, FR-31, §8.25) — Bring to Front/Forward, Send Backward/to Back within a layer, plus the agent's `arrange_object` tool — shipped 2026-08-24 as v1.27.3 (PR #340): `ui/canvas/arrange.py::build_arrange_command` is the one apply path for every arrange surface; the per-layer z-refresh never writes a `stack_order` rank (only `addItem`/`MoveToLayerCommand`/`ArrangeItemsCommand`/the post-load renumber do); the fix also closes a proven pre-existing bug where every save/load round-trip inverted same-layer stacking. Issue #342 (Google Maps Preferences key and robust picker lifecycle, PR #344) shipped 2026-08-31 as v1.27.5, and the JavaScript API redesign (#346, PR #348) shipped 2026-09-01 as v1.27.6 — JS view capture for EEA-blocked satellite imports plus ADR-019 addendum #346. Follow-up #347 (pan-grid view capture: capture-profile phase, multi-frame settle/advance/retry choreography, world-pixel seam-free stitching, `pick_capture_zoom_and_grid` 3×3 cap, one baked attribution strip) shipped 2026-09-02 as v1.27.7 — FR-IMG-11, ADR-019 addendum #347. Issue #356 (Windows Defender flagged the exe as `Trojan:Script/Wacatac.C!ml`, a PyInstaller ML-heuristic false positive) shipped 2026-09-15 as v1.27.9 (PR #357): `release.yml` now attests build provenance (Sigstore via `actions/attest-build-provenance`) for the installer, the raw app exe, and `SHA256SUMS.txt`, verifiable with `gh attestation verify`. This proves source provenance, not Authenticode signing; SmartScreen/Defender suppression remains the open cert-cost question. See ADR-044, §11.1/§11.2.
...
- **#365 and #366 are implemented** on `feature/issue-366-365-agent-onboarding-and-surface` (draft PR; senior-review and the owner's manual test still pending). #366 rewrites `services/ai_client_onboarding.py` around a frozen `ClientTarget` registry — clients are **data**, so the dialog gained **OpenCode, Codex, and Gemini CLI** with no UI change — and makes the **read-only connect URL the default hand-out**, because the single "Copy URL" button used to copy the *write-enabled* URL, which is how a live write credential ended up pasted into a chat. Three serializers keyed on `syntax`; **TOML is appended surgically** (no `tomli-w`: a full re-serialise would destroy the user's own comments in a file OGP does not own) and **JSONC needs a tolerant reader**, or a commented `opencode.jsonc` fails closed into a dead end. #365 adds token-gated `new_plan` / `open_plan`, both reusing the GUI's own paths (extracted as `_new_project_document` and a raising `_load_project_file`), and reverses the layer-lock policy. **Read ADR-035's #366 addendum and ADR-034's #365 addendum before touching either area** — both record decisions that are not obvious from the code, including two things deliberately left unchanged (OpenCode's 5 s timeout, and the token still riding the URL rather than a header, because header transmission on tool calls is unmeasured).
