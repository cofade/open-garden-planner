...
| 🟧     | C3    | Vertical gardening + container gardening — **split into two PRs**. **C3a ✅ (this branch):** Containers (rect/round) + Wall Planters as soil-bearing plant-parents + a basic Trellis (plant-parent, no soil). New `ObjectType`s on existing shape items (like `RAISED_BED`); **predicate split** `is_bed_type` (soil-capable, now incl. containers) vs new `is_plant_parent_type` (+ trellis) — see **ADR-031**. Qt-free `core/container_model.py` (soil-volume/watering/capacity); properties **Container** section (material/drainage/height/volume + live watering hint); **capacity-overrun badge**; shopping-list volume by container height. Additive `metadata` (no FILE_VERSION bump). New **"Vertical & Container"** toolbar category (now 11). See **FR-24**. **C3b ⬜ (follow-up):** full Trellis context menu (`supports_soil=False`) + **1-D vertical spacing** (`_check_spacing_group` projects onto the trellis axis) — needs product sign-off on vertical semantics. |
