...
| ✅     | #226 | Modern sidebar **hover-peek + click-to-pin accordion** (ADR-030, refines ADR-005; FR-22/FR-UI-SIDEBAR; arc42 §8.17). New `ui/widgets/panel_stack.py` `SidebarController` owns all layout + a per-panel state machine (`PanelState ∈ {COLLAPSED, PEEKING, PINNED}`, `PinSource ∈ {USER, SELECTION}`). All 9 panels start **collapsed** every session (**no pin/panel-state persistence** — `UiStateStore.save/restore_panel_state` removed, `tests/unit/test_ui_state.py` deleted). **Hybrid layout** (not one splitter): collapsed/peeking bars live in a `QVBoxLayout`; pinned panels move into a lazy vertical `QSplitter` (canonical order, equal-share re-equalized on every pin/unpin, 5px grabbable handle via `#pinnedSplitter` QSS) + trailing `addStretch()`. **Hover** peeks a bar open in place (asymmetric debounce: open ~140ms / close ~220ms; re-enter cancels pending close → anti-flicker); **click** title pins/unpins. The 3 contextual panels **auto-pin** on selection via `set_selection_pinned(key, on)` (only `SELECTION`-source pins are torn down on clear; a user click upgrades to `USER`); the selection updaters keep their content-update calls. `CollapsiblePanel` gains `_HeaderFrame` (`hover_enter`/`hover_leave`/`pin_toggled`), `header`/`header_height()`/`set_visual_state()` (dynamic `panelState` QSS prop — re-polish **both** panel + header). Gotchas (see §8.17): release `maximumHeight` clamp **before** splitter insert; reset `minimumHeight(0)` **before** collapse clamp; compute canonical splitter index **before** flipping to PINNED; defer `setSizes` via `singleShot(0)` (re-defer if height 0). New tooltips under `CollapsiblePanel` i18n context. Tests: `tests/unit/test_sidebar_controller.py`, extended `tests/ui/test_collapsible_panel.py`, `tests/integration/test_sidebar_accordion.py`; `test_sidebar_order.py` reads order from `controller.panels()`. |
