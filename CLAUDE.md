...
| Status | US    | Description                                                      |
| ------ | ----- | ---------------------------------------------------------------- |
| ✅     | C1    | Harvest tracking / yield log — per-plant/bed **"Log Harvest…"** dialog (Entry + History, undoable Add/Edit/Delete) keyed by item UUID under additive `.ogp` `harvest_logs` (no FILE_VERSION bump); each `HarvestHistory` caches `species_key`/`species_name`. Garden-wide **Harvest** dashboard tab (**Ctrl+6**) driven by Qt-free `services/harvest_aggregation` (per-species/year/unit totals) + CSV export + optional PDF "Harvest summary" page. Each harvest auto-creates a **pin-less** `harvest`-tagged `JournalNote` (new `JournalNote.tags`). Carries forward on new-season rollover (year-over-year). Mirrors the pest-log architecture (US-12.7). See **FR-23**. |
...
| ✅     | C3    | Vertical gardening + container gardening (PR #236, C3a + C3b). **C3a:** Containers (rect/round) + Wall Planters as soil-bearing plant-parents. New `ObjectType`s on existing shape items (like `RAISED_BED`); **predicate split** `is_bed_type` (soil-capable, now incl. containers) vs new `is_plant_parent_type` (+ trellis) — see **ADR-031**. Qt-free `core/container_model.py` (soil-volume/watering/capacity); properties **Container** section (material/drainage/height/volume + live watering hint); **capacity-overrun badge**; shopping-list volume by container height. Additive `metadata` (no FILE_VERSION bump). New **"Vertical & Container"** toolbar category (now 11). **C3b:** Trellis context menu via `build_bed_context_menu(supports_soil=False)` (Pest/Harvest/Succession, no Grid/Soil-test; menu guard moved to `is_plant_parent_type`) + **1-D vertical spacing** (`_update_spacing_overlaps` feeds `_check_spacing_group` a distance callable that projects onto the trellis's rotation-aware long axis). See **FR-24**, ADR-031 addendum. **Manual-test fix:** gallery thumbnails now follow each object's shape (round container → circular icon; trellis/container/wall-planter → rectangular) — the trellis' `SOLID` fill has no texture, so it was falling through to the round fallback; added `render_color_rect_thumbnail` + shape-routing in `gallery_data._vertical_container_items`. |
| ✅     | C4    | Parametric blocks / smart symbols (PR #236). Reusable parametric blocks (raised-bed-with-rows, pergola, fence panel, gabled greenhouse, 3-section compost bay) defined in versioned **JSON** under `resources/data/smart_symbols/` (+ user `<app-data>/smart_symbols/`, drop-a-file extensible). Declarative geometry DSL (`rect`/`line`/`polyline`/`polygon`/`circle` + `repeat`, arithmetic expressions) evaluated by a safe no-`eval` `core/parametric_eval.py`; Qt-free `models/smart_symbol.py` emits geometry specs. **`SmartSymbolItem(GroupItem)`** regenerates children on param edit (preserves pos/rotation/layer); new **Smart Symbols** sidebar panel + properties param widgets (undoable). Serialized as `group` + `smart_symbol` metadata (**no FILE_VERSION bump**, old-app graceful degrade, cached-geometry fallback on missing/version-mismatch). DXF **BLOCK + INSERT** export. **UI deferred:** the sidebar **panel is hidden** (one-line `set_panel_visible("smart_symbols", False)` in `application.py`); engine/persistence/DXF/properties-editing ship and are tested, symbols in an existing `.ogp` still regenerate. See **ADR-032**, **FR-25** (UI status note), §8.18. |
...
| ✅     | D1.3  | Vision tool (v1.23.0, PR #242) — `render_canvas_image(x?, y?, width?, height?, layers?, image_width_px?)` returns a PNG of the live canvas + `RenderMeta`, reusing `services/scene_rendering.render_scene_region` (the `ExportService.export_to_png`/PDF pipeline — no new rendering logic). New Qt-**touching** `agent_api/render.py` (the one non-Qt-free module in the package); `resolve_image_pixel_size` double-clamps width + derived height to `[128, 2048]`; `AgentProviders.render` is the first *parametrized* provider (one atomic main-thread hop). Two facts empirically verified against mcp 1.28.1: `Image` isn't pydantic-representable, so `@mcp.tool(structured_output=False)` is required (a naive `list[Image \| RenderMeta]` annotation crashes `build_server()`; `Image` must also be a genuine top-level import in `server.py`, not function-local, since `inspect.signature(func, eval_str=True)` resolves stringified annotations via the tool's own `__globals__`) — and `render_scene_region`'s `y_flip=True` inverts the rendered pixel Y-axis relative to D1.2's scene frame (correction formula documented on `RenderMeta.px_per_cm`, pinned by a dedicated coordinate-frame test). **Manual-test fix:** `layers` was originally subtractive-only (could hide, never show) — a layer the user had toggled off in the live Layers panel stayed invisible even when explicitly requested; `_hidden_layers_not_in` now snapshots every layer-bearing item's original visibility, forces the full requested set, and restores the exact original state, so an agent asking for a hidden layer gets it without any lasting UI side effect. Independent review found no P0/P1s; three P2 nits (restore-ordering robustness, a docstring precision fix, and an aspect-ratio-at-clamp-boundary note) addressed. No FILE_VERSION change. See ADR-034 addendum, §8.19, FR-AGENT-09. |
| ✅     | D1.4  | Export/save tools (v1.24.0, PR #243) — `save_plan(file_path?)`, `export_pdf(file_path?, paper_size?, orientation?)`, `export_dxf(file_path?)`, `export_csv(kind, file_path?)`, the first Agent API tools with a **filesystem side effect** — each calls the exact same service the GUI's File > Export/Save menu already uses (`PdfReportService`/`DxfExportService`/`ExportService`/`ShoppingListService`/`ProjectManager.save`), no new export logic. New `agent_api/exports.py`: `resolve_export_path` reuses the `app/paths.py` #199/#204 safe-path chokepoint when no `file_path` given, else force-applies the suffix and requires the parent directory already exist. `save_plan` with no path and no project open raises rather than inventing a filename (mirrors the GUI's own Save-As requirement); given a path it becomes the project's file going forward, mirroring `application._save_to_file` exactly (incl. the issue-#178 stale shopping-list-price prune). No token auth needed — none of the four can make an edit an agent isn't otherwise capable of; that gate stays reserved for D2. Independent review found two real P1s (both fixed): `previous_file_path` returned the literal string `"None"` instead of JSON `null` on a never-saved project's first save-as, and `paper_size`/`orientation` weren't `Literal`-typed (silently falling back to A4/landscape on a bad value). **Bonus fix found while verifying the frozen exe:** `installer/ogp.spec` excluded `"unittest"`, but `ezdxf`'s query support transitively needs it via `pyparsing.testing` at import time — this silently broke DXF export/import (US-12.3/12.4) in any freshly-built frozen exe, not just this PR's `export_dxf`. **Real-world dogfood test** (an independent Claude instance driving the live MCP server against a real 53m×41m/3-bed/11-plant plan) confirmed all four tools end-to-end, including `previous_file_path`'s round-trip-restore use case. No FILE_VERSION change. See ADR-034 addendum, §8.19, FR-AGENT-10. |
| ✅     | D1.5  | Agent API resources + read-analysis prompts (v1.24.1, PR #250) — the last leg of the epic's locked "Tools + resources + prompts" MCP surface design. 5 read-only `@mcp.resource()`s: `garden://plan` (curated `PlanSummary`), `garden://plan/raw` (uncurated snapshot dict), `garden://canvas.png` (full-canvas PNG), `garden://diagnostics`, `garden://species` (bundled DB, 118 records — the one genuinely new data source, needs no `AgentProviders` field/main-thread hop since it's static Qt-free bundled data). 2 read-analysis `@mcp.prompt()`s: `audit-plan` (layout + diagnostics → prioritized improvements) and `describe-garden` (narrative description), both composed by a new pure `agent_api/prompts.py` (Qt-free text builders, capped at 50 objects for `describe-garden`). Almost entirely registration/wiring over D1.1–D1.4's existing providers — no new business logic except the two text builders. Verified directly against `mcp` 1.28.1 source: resources are simpler than tools (a `bytes` return + `mime_type` auto-produces `BlobResourceContents`, no `Image`/`structured_output=False` workaround needed; non-`str`/`bytes` returns auto-serialize via `pydantic_core.to_json`). No `FILE_VERSION` change; no token auth needed (read-only, same as D1.1–D1.4). First senior-review pass found no P0/P1s, three P2 nits fixed (species-resource thread offload, a duplicated "(unsaved)" string, the 50-object cap). **A second, independent senior-review pass** (run fresh in an isolated worktree during finalize-us) caught one real P1 the first pass missed — `docs/05-building-block-view` never got a black-box row for the new `prompts.py` module and its `server.py` row still described only the 14 tools, not the 5 resources + 2 prompts — plus README's stale version pin and an "all shipped" overclaim while Package D was still landing; both fixed same-session, plus two previously-uncovered branches (>50-object cap, saved-file-with-pending-edits) got regression tests. **Real-world dogfood test**: an independent Claude instance connected a real MCP client to a live running instance with an actual open plan (3 beds/11 plants/29 shapes) and read every resource + invoked every prompt end-to-end — confirmed a genuine live `soil_mismatch` diagnostic surfaced correctly, and `describe-garden` named the plan's actual custom-named objects (not placeholder text). See ADR-034 addendum, §8.19, FR-AGENT-11. |
| ✅     | D1.6  | AI client onboarding (v1.24.2, PR #251, issue #240) — in-app "Connect AI Assistant…" dialog (Help menu + Preferences → Agent API button) shows the connect URL and, per detected client, either one-click registers it or hands back a guided copy-paste snippet. New Qt-free `services/ai_client_onboarding.py`: **Cursor** via a direct atomic JSON merge into `~/.cursor/mcp.json`; **Claude Code** via the `claude mcp add --transport http --scope user` CLI (preferred over hand-editing `~/.claude.json`'s nested per-project schema); **Claude Desktop is detection-only** — verified live against `modelcontextprotocol.io`'s own docs that its local config only documents stdio servers, so a plain local HTTP endpoint is registered through the app's own Settings → Connectors → "Add custom connector" UI instead (auto-writing a guessed schema, or the `mcp-remote` npm bridge workaround, rejected as unnecessary risk). Introduces this codebase's **first atomic-write pattern** (temp file + `os.replace`, backup-before-overwrite, preserves every other key/server) since every prior JSON writer here only ever wrote its own file. **Four senior-review rounds**, each catching a real issue before the next: (1) unguarded `subprocess.TimeoutExpired`/`OSError` from the Claude Code CLI call reaching an unguarded Qt slot; (2) that install blocking the GUI thread up to ~45s with zero visual feedback (added a WaitCursor + button-disable, mirroring `preferences_dialog._test_api`'s existing blocking-call precedent); (3) **the significant one** — the Preferences dialog's Connect button derived its URL from comparing widget state to *saved settings*, not from whether the server was actually **running**, so a saved-but-dead port (e.g. a port conflict at launch) would have been silently registered into a client's config as if it worked; fixed with one shared `GardenPlannerApp.agent_api_running_url()` accessor both entry points now call; (4) a stale doc line describing the just-removed pattern, plus a direct regression test added for the accessor itself. No token auth needed (registering a client isn't scene-mutating); no `FILE_VERSION` change. **Follow-up (issue #253):** onboarding no longer needs the `claude` CLI — when it's absent, Claude Code is registered by a direct **fail-closed** atomic merge into `~/.claude.json`'s top-level `mcpServers` (`type: "http"`, preserves OAuth/trust); Claude Desktop's row is now an honest "can't reach localhost — use Claude Code/Cursor" note (no snippet); and the Connect dialog's rows scroll so a revealed snippet no longer clips behind Close. See ADR-035 (issue-#253 addendum). |

| ✅     | D2.0  | Agent write path — token gate + first write slice (v1.24.3, PR #252) — the D2 prerequisite ADR-033 named. Two scene-mutating tools, **`move_object(item_id, dx, dy)`** + **`delete_object(item_id)`**, behind **two independent guards**: an off-by-default **"Allow AI assistants to edit the plan"** toggle (`agent_api_writes_enabled`) AND an auto-generated **bearer token** (`agent_api_token`, `secrets.token_urlsafe(32)`). Write tools are **registered only when both are set** (else absent from the tool list) and every call must present the token **in the connect URL as a `?token=` query param OR as an `Authorization: Bearer <token>` header** (see the URL-delivery note at the row's end). **Reads stay open** (loopback trust), so the gate is **per-tool, not a blanket middleware** that would break read-only D1 clients: a pure-ASGI `_bearer_token_middleware` reads each request's token from the header or the `?token=` query string into a `ContextVar`, and each write handler runs `_require_write_auth` (constant-time `secrets.compare_digest`, `isascii()`-guarded so a non-ASCII header fails closed instead of raising) in the same request task **before** the `anyio.to_thread.run_sync` hop — verified end-to-end against `mcp==1.28.1` with `stateless_http=True` (chosen over a `ctx: Context` header read to avoid depending on FastMCP internals). Each write hops to the main thread, resolves the item by UUID (`scene.find_item_by_id`), and runs `canvas_view.command_manager.execute(cmd)` — matching GUI **behaviour**, not merely a `Command` class. **`move_object` mirrors `CanvasView`'s whole drag-release sequence, not just its simplest branch** — a bed/container/trellis's contained plants move with it (`AlignItemsCommand` over item+children, mirroring `_propagate_bed_children_during_drag`'s release-time commit), and a moved plant's bed membership is re-evaluated afterward (`SetParentBedCommand` on a boundary crossing, mirroring `_update_plant_bed_relationships`) — **an independent review (fresh worktree) caught this as a P0 in the first cut**, which used a bare `MoveItemsCommand([item], delta)` that silently abandoned a bed's plants and left stale parent/child links for soil-mismatch diagnostics to act on, with no way for an agent to correctly relocate a plant at all. So one agent move is **one undo step for a lone item, two when it also reparents a plant** — never more; `WriteResult` gained `children_moved`/`bed_membership_changed`/`new_parent_bed_id`. **`delete_object` mirrors `CanvasView._delete_selected_items`'s full orchestration**, not just `DeleteItemsCommand` (already detaches contained plants) — a **second independent review** caught two more gaps: constraints referencing the deleted item are removed (`RemoveConstraintCommand`, else the constraint graph keeps a dangling UUID) and a `HOUSE`'s linked roof ridge is deleted with it (`metadata["ridge_item_id"]`, else orphaned); `WriteResult` gained `linked_items_deleted`/`constraints_removed`. **Both write tools now refuse (raise) — in the shared `_resolve_agent_item` chokepoint, so future D2 tools inherit it — rather than silently mishandle cases the agent could reach by direct-UUID resolution but the GUI forbids**: an object (or, for `move_object`, a bed child) in any geometric constraint (replicating `CanvasView`'s live solver for a one-shot call is deferred); a journal pin (delete needs a `ProjectData` note-record prune no bare `DeleteItemsCommand` performs); an item on a **locked layer** (a **third review** catch — the GUI enforces the lock via `ItemIsSelectable`/`ItemIsMovable` flags that selection-based GUI edits respect but a UUID lookup bypasses); and an individual **group member** (a raw snapshot exposes its id, but `moveBy` on a `QGraphicsItemGroup` child displaces it within the group — address the group itself; moving the group *itself* cascades to members natively since `GroupItem`/`SmartSymbolItem` are real Qt groups, unlike the logical bed↔plant link). Verified `ridge_item_id` is the only metadata-linked association, and spacing/companion/soil diagnostics recompute off `QGraphicsScene.changed` (fired by agent moves too). Token surfaced (Copy/Regenerate) in Preferences → Agent API — a **Copy/Regenerate drift note** now appears when the running server's actual token differs from what Regenerate just persisted to settings (another review catch) — and injected into client configs by the D1.6 Connect dialog. **URL-token delivery (manual-test finding):** the token ships **in the connect URL as `?token=<token>`**, not an HTTP header — Claude Code (the primary client) has a known, unfixed bug where it stores a configured header but doesn't transmit it on tool-call requests for `type: http` servers (anthropics/claude-code#50464 / #28293), so header-only writes silently failed despite `claude mcp get` showing the header. The URL is always sent (it's the request target), so `ai_client_onboarding.url_with_token` embeds the secret there for every client and the `--header` injection is dropped entirely (also killing the variadic-`--header`-ordering footgun); the server still *accepts* a header for back-compat. Threat model unchanged (a caller still needs the token); accepted tradeoff is token visibility in loopback request logs. Tests: `test_agent_api_auth.py` (header **and** `?token=` extraction, header-wins), `test_agent_api_writes.py` (real MCP client: authenticated move/delete via header AND via URL token mutate + undo restores + one undo step; unauthenticated call errors, scene untouched), `test_ai_client_onboarding.py::TestUrlWithToken`/`TestWriteTokenThreading`, connect-dialog token-in-URL tests, plus bed-with-children-propagates / plant-crosses-bed-boundary (both directions) / constrained-object-refused / constraint-removed-on-delete / linked-roof-ridge-deleted / journal-pin-rejected / locked-layer-refused / group-member-refused regression tests. No `FILE_VERSION` change. See ADR-036 (+ URL-delivery addendum), §8.19/§8.11, FR-AGENT-13. **Follow-up (issue #267):** the agent API described the scene frame as "+y down" though OGP is CAD Y-up (ADR-002) — an agent told "move south" moved objects north; corrected every agent-facing coordinate description to state +y = north (a negative dy moves south), pinned by a regression test. Pure description fix — the move/read/render math (incl. the test-pinned render pixel formula) was already correct. |

**Package D — D1 complete; D2 underway.** US-D1.1…D1.6 shipped; **D2.0** (token gate + first write slice) shipped. Next in D2: `create_object` (reuse the loader's `_deserialize_item_core` + `CreateItemCommand`), `resize_object`/`rotate_object`, `assign_species`/`set_parent_bed`/`set_layer`, then geometry escape hatches — all behind the D2.0 gate. Or **Phase 14 (v2.0)** — 3D Visualization & Sun/Shade. See `docs/roadmap.md`.
...
| ✅     | #226 | Modern sidebar **hover-peek + click-to-toggle accordion** (ADR-030, refines ADR-005; FR-22/FR-UI-SIDEBAR; arc42 §8.17). New `ui/widgets/panel_stack.py` `SidebarController` owns all layout + a per-panel state machine (`PanelState ∈ {COLLAPSED, PEEKING, PINNED}`, `PinSource ∈ {USER, SELECTION}`). All 9 panels start **collapsed** every session (**no pin/panel-state persistence** — `UiStateStore.save/restore_panel_state` removed; `tests/unit/test_ui_state.py` trimmed to the surviving geometry/splitter round-trips). **First cut used a bottom `QSplitter` (reparent on pin, equal-share, draggable) and failed manual testing** → reworked (ADR-030 addendum): **all panels live in ONE `QVBoxLayout` inside a `QScrollArea`, never reparented** (so opening one never reorders the list — the reported bug); open panels **fill the surplus space** (content-weighted layout stretch + `min=content`; one fills the sidebar, several share by content size, overflow **scrolls**); **open/close is animated** (`CollapsiblePanel.animate_expand/collapse` — `QPropertyAnimation` on `maximumHeight`, ~160ms `InOutCubic`; `expand_now`/`collapse_now` = instant). **Click toggles** open↔closed; a click on a `SELECTION`-opened panel **closes it and sets `selection_dismissed`** so a same-selection re-notify (scene move/undo) won't reopen it; `reset_selection_dismissals()` (called from `_on_selection_changed` before the `_update_*_panel` slots) re-enables auto-open on a genuine selection change. `USER` pins survive selection clear. The 3 contextual panels (plant_details/companion/crop_rotation) are **hidden entirely until a matching item is selected** (`set_panel_visible(key, relevant)` — orthogonal to open/collapse, restores pre-US-226 `setVisible(False)`; hidden at startup, toggled by the selection updaters alongside `set_selection_pinned`). **Hover** peeks (asymmetric debounce open ~140 / close ~220ms; re-enter cancels close). `CollapsiblePanel` gains `_HeaderFrame` (`hover_enter`/`hover_leave`/`pin_toggled`), `header`/`header_height()`/`set_visual_state()` (dynamic `panelState` QSS — re-polish **both** panel + header). `SidebarController` exposes `panel(key)`/`panel_keys()`/`is_open()`; the `application._tracked_panels` mirror is gone. Tooltips: `Click to open`/`Click to keep open`/`Click to collapse`. Tests: `tests/unit/test_sidebar_controller.py` (incl. order-stable-on-open, single-click-closes-selection-panel, dismissed-until-reselection), extended `tests/ui/test_collapsible_panel.py` (animated/instant end-states), `tests/integration/test_sidebar_accordion.py`; `test_sidebar_order.py` reads `controller.panel_keys()` + asserts layout order. |
