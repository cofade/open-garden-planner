...
| ✅     | #206 | Properties-panel incremental updates: `set_selected_items` no longer tears down + rebuilds the whole form on every `command_executed`/`can_undo/redo_changed` signal (the root cause behind #200's focus loss, previously only patched with a focus guard). It now computes a structural **identity** (`_compute_identity`: item id + class + `object_type` + bed `child_item_ids` / plant `parent_bed_id`) and rebuilds **only** on a genuine selection/structure change; an unchanged selection pushes fresh model values into the live widgets via registered **refreshers** (`_register_refresh` next to each widget; `_refresh_field_values` runs them, each skipping a focused widget via `_is_focused` and writing under `blockSignals`). A naive "skip rebuild on same selection" was rejected — it would leave Position/Size/colour stale after a canvas drag/undo/redo; the refresh path keeps them correct. The old focus guard stays as a **defensive backstop** (no longer load-bearing → new fields need not be whitelisted in it). Unguarded sibling `plant_database_panel` got a matching `_holds_field_focus()` backstop. New `tests/integration/test_properties_panel_incremental.py`. See §11.4. |
