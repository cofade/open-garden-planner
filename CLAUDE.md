...
| ✅     | D2.0  | Agent write path — token gate + first write slice (PR pending) — the D2 prerequisite ADR-033 named. Two scene-mutating tools, **`move_object(item_id, dx, dy)`** + **`delete_object(item_id)`**, behind **two independent guards**: an off-by-default **"Allow AI assistants to edit the plan"** toggle (`agent_api_writes_enabled`) AND an auto-generated **bearer token** (`agent_api_token`, `secrets.token_urlsafe(32)`). Write tools are **registered only when both are set** (else absent from the tool list) and every call must send `Authorization: Bearer <token>`. **Reads stay open** (loopback trust), so the gate is **per-tool, not a blanket middleware** that would break read-only D1 clients: a pure-ASGI `_bearer_token_middleware` stashes each request's token in a `ContextVar`, and each write handler runs `_require_write_auth` (constant-time `secrets.compare_digest`) in the same request task **before** the `anyio.to_thread.run_sync` hop — verified end-to-end against `mcp==1.28.1` with `stateless_http=True` (chosen over a `ctx: Context` header read to avoid depending on FastMCP internals). Each write reuses the GUI's own path exactly: an `AgentProviders` callable hops to the main thread, resolves the item by UUID (`scene.find_item_by_id`), builds one existing `Command` (`MoveItemsCommand`/`DeleteItemsCommand`), and runs `canvas_view.command_manager.execute(cmd)` — **one agent write = one undoable step** that marks the plan dirty (invariants #3/#4/#13; no parallel mutation path). Token surfaced (Copy/Regenerate) in Preferences → Agent API and injected into client configs by the D1.6 Connect dialog as the `Authorization` header (Cursor `headers` key; Claude Code `--header` flag). Tests: `test_agent_api_auth.py` (gate + conditional registration + middleware extraction), `test_agent_api_writes.py` (real MCP client: authenticated move/delete mutate + undo restores + one undo step; unauthenticated call errors, scene untouched). No `FILE_VERSION` change. See ADR-036, §8.19/§8.11, FR-AGENT-13. |
