...
> **Scope (spike):** US-D1.1 ships the server (on by default, read-only), the enable/disable toggle, the main-thread marshaling boundary, and ONE read tool (`get_plan_summary`) + the PyInstaller bundling. End-user client onboarding (registering the URL into Claude Desktop/Cursor/Claude Code) is a follow-up (D1.6). D1.2 (read/query), D1.3 (vision), and D1.4 (export/save) round out the read-only-plus-file-export surface below; resources/prompts (D1.5) and write/domain tools (D2/D3) follow. See ADR-033, ADR-034, §8.19.
