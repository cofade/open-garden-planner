...
- **FR-PLANT-18** *(issue #297)*: Confirming a plant search result in `PlantSearchDialog` (OK or double-click) fetches the provider's full per-species detail record via `PlantAPIManager.get_by_id()` and merges its enrichment fields onto the search result before it is assigned to a plant — identity fields (`common_name`, `scientific_name`, `family`, `genus`, `image_url`, `thumbnail_url`) keep the search result's value unless the detail response actually improves on it, since a wholesale swap was found (senior review round 3) to silently blank identity fields the detail response validly omits. Search/list endpoints can be structurally sparser than detail endpoints for the same provider — confirmed live for Trefle's `/plants/search`, which omits `growth`/`specifications`/`foliage` entirely, so `sun_requirement`, `water_needs`, `ph_min`/`ph_max`, `nutrient_demand`, `foliage_color`/`foliage_texture`, and (also detail-only) `max_height_cm`, `growth_rate`, `description` would otherwise stay at their unset defaults for every Trefle-sourced plant regardless of parser correctness. The fetch runs once, on confirm, not per browsed result (rate-limit consideration); is skipped when `data_source == "custom"` or `source_id` is empty (local library entries and unidentifiable results have no online detail endpoint to fetch); and on failure — a raised error, or a successful response whose `source_id` doesn't match the one requested (empty, truncated, or otherwise not the record asked for) — shows a translated warning and falls back to the original sparse result rather than silently reproducing the bug or blocking the user's selection. Deliberately does **not** reject on an empty/missing `common_name` alone: Trefle omits it for many genuinely valid, scientific-name-only species, and a first cut of this fix wrongly discarded fully-populated detail records on that basis (caught in senior review round 2).
