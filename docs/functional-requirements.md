...
- **FR-PLANT-18** *(issue #297)*: Confirming a plant search result in `PlantSearchDialog` (OK or double-click) fetches the provider's full per-species detail record via `PlantAPIManager.get_by_id()` and replaces the sparse search-result data before it is assigned to a plant. Search/list endpoints can be structurally sparser than detail endpoints for the same provider — confirmed live for Trefle's `/plants/search`, which omits `growth`/`specifications`/`foliage` entirely, so `sun_requirement`, `water_needs`, `ph_min`/`ph_max`, `nutrient_demand`, and `foliage_color`/`foliage_texture` would otherwise stay `UNKNOWN`/`None` for every Trefle-sourced plant regardless of parser correctness. The fetch runs once, on confirm, not per browsed result (rate-limit consideration); is skipped for `data_source == "custom"` (local library entries have no online detail endpoint and are already complete); and falls back to the original sparse result on fetch failure rather than blocking the user's selection.
