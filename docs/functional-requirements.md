...
- **FR-PLANT-17** *(issue #213)*: Assigning a database species to an **existing** generic plant — via the plant database panel (Load Custom / Create Custom / Save to library) or the Plants-menu species search (`application.py`) — writes `metadata["plant_species"]` **and** refreshes the on-canvas spacing circle (which derives from the species' `max_spread_cm` via `effective_spacing_radius()`). The assignment is a single undoable step (`ApplySpeciesCommand`, one undo unit, dirties the document). If the user has a manual spacing-radius override that differs from the database value, a prompt offers **Apply database values** (clears the override so the DB value cascades) or **Keep custom values** (override preserved); no prompt appears when there is no differing override. The drawn circle diameter is left unchanged (consistent with FR-PLANT-15 drop behaviour — only the spacing circle and metadata update).
