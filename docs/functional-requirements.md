...
- **FR-PLANT-17** *(issue #213)*: Assigning a database species to an **existing** generic plant — via the plant database panel (Load Custom / Create Custom / Save to library) or the Plants-menu species search (`application.py`) — writes `metadata["plant_species"]` **and**, on *Apply database values*, resizes the drawn footprint so its **diameter equals the species' `max_spread_cm`** (the visible change the issue requires) while keeping the plant centred, and clears any manual spacing override so the database value cascades. The assignment is a single undoable step (`ApplySpeciesCommand`, one undo unit, dirties the document — metadata, override, and footprint radius all revert together). When applying would change the placed size or a manual spacing override differs from the database value, a prompt offers **Apply database values** (default) or **Keep custom values** (placed size and override preserved); no prompt appears when nothing differs (footprint already equals `max_spread_cm` and no override) or for the library-save path (`prompt=False`). Species with no usable `max_spread_cm` only update metadata (no resize). The footprint is resized via `CircleItem.set_radius_centered()`.
