...
## FR-20: CAD Precision — Curve Tools, Corner Edits, Reference-Point Snaps, Paper Space (Phase 13, Package B)

### FR-20.1 Curve Tools
- **FR-DRAW-08** (US-B1): Cubic Bezier pen tool. Click-drag per anchor — drag direction sets the outgoing handle; the incoming handle is mirrored automatically for a smooth tangent default. Double-click or Enter commits the curve. ESC cancels. Existing curves enter edit mode on selection and expose drag-able anchor + handle widgets that re-use the polyline vertex-edit infrastructure (ADR-022). Shortcut: `B`.
- **FR-DRAW-09** (US-B2): 3-point arc tool. Click start → through-point → end; the unique circular arc through the three points is computed via the circumcenter (`core/cad_geometry.arc_from_three_points`) and rendered as an `ArcItem`. If the three points are collinear (twice the signed triangle area ≤ 1.0), the tool falls back to a 2-vertex polyline and a status-bar note. Shortcut: `A`.
...
### FR-20.4 Paper Space
- **FR-LAYOUT-01** (US-B7): Layout tab (Ctrl+4) — a dedicated paper-space scene next to the existing Garden Plan / Calendar / Seed Inventory tabs. Lazy-created on first visit so a fresh window doesn't pay the cost up front. The tab hosts one printable page with one viewport, one title block, and one scale bar.
- **FR-LAYOUT-02** (US-B7): Viewport — a rectangular "window" onto a region of the model canvas. Rendered through `services/scene_rendering.render_scene_region()` with overlay / construction / scale-text bookkeeping handled by context managers, then cached as a `QPixmap`. The cache invalidates on `source_scene.changed` so model edits propagate to the layout automatically. Scale factor (paper-cm / model-cm) is the ratio of the viewport's paper-rect width to its source-rect width; changing the scale resizes the source rect around its centre, leaving the viewport's paper-space placement untouched.
- **FR-LAYOUT-03** (US-B7): Title block — three auto-filled fields: Project (project filename stem), Date (today's ISO date), Scale (1:N derived from the primary viewport).
- **FR-LAYOUT-04** (US-B7): Scale bar — labelled tick bar with five stops; total length stays fixed in paper-cm, labels switch units (cm / m) based on the model-space distance. Recomputes automatically when the viewport scale changes.
- **FR-LAYOUT-05** (US-B7): Persistence — paper layouts are serialised under the top-level `paper_layouts` key in the `.ogp` file (`FILE_VERSION` 1.4). v1.3 files load with an empty layouts list (regression-tested), so old projects open cleanly.
- **FR-LAYOUT-06** (US-B7): Paper Space MVP scope — exactly one layout, one page, one viewport per project. Multi-page, multi-viewport, multiple layouts, and custom title blocks are explicit out-of-scope; the file format leaves room to grow without breaking compatibility.
