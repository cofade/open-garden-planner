...
- **FR-UI-SIDEBAR-04**: An open panel grows to its **content height**; when the open panels exceed the sidebar height the sidebar **scrolls**. (There is no equal-share split or draggable divider.)
- **FR-UI-SIDEBAR-05**: Selecting a single plant **auto-opens** Plant Details and Companion Planting; selecting a single bed auto-opens Crop Rotation. The user can **close an auto-opened panel with a single click**, and it stays closed for that selection; a different selection re-opens it. Clearing the selection closes the auto-opened panels, but a panel the user opened manually stays open.
