...
- **FR-PLANT-19** *(issue #302)*: A plant search that every reachable database answers with **zero matches is not an error** and must never be reported as one. `PlantAPIManager.search()` raises `PlantAPIError` only when *no* configured provider answered at all (every one raised) and the custom library had no hit; if at least one provider answered — even with an empty list — the honest result `[]` is returned. `PlantSearchDialog` then shows a warning-coloured status *"No plants matched '{query}'. Try another spelling or the scientific name."* with **no modal dialog**, or *"No plant databases are configured. Add API credentials in Preferences to search online."* when `PlantAPIManager.configured_source_count == 0`. The red *"Search failed: …"* status plus the *"check your internet connection and API credentials"* message box is reserved for a genuine technical failure. (Before this, a query for a variety absent from every database — the reporter's *mahachanok* mango — produced the failure dialog with the credentials hint even though connection and credentials were fine; the same misreport class the #294 Preferences "Test" button had.) Pinned by `tests/integration/test_plant_search_zero_results.py` and `tests/unit/test_plant_api_manager_search.py`.
...
- **FR-UI-13** *(issue #304)*: Across the main toolbar and the constraint toolbar **exactly one tool button is highlighted at any time — the active tool's**. The two toolbars own separate exclusive `QButtonGroup`s, so the app reconciles them itself: `ToolManager` emits `tool_type_changed(ToolType)` alongside the display-name `tool_changed(str)`; `GardenPlannerApp` forwards it to both toolbars' `set_active_tool()`, each of which unchecks its own group when the active tool is not one of its buttons. (Before this, the sync was keyed on the *translated display name* and knew only three constraint tools, so e.g. activating **Equal Size** left **Select** highlighted too — the user could then only reach objects via right-click and reasonably read the stuck highlight as the bug.) Pinned by `tests/integration/test_toolbar_highlight_sync.py` (parametrised over every constraint-toolbar tool plus the main-toolbar tools).
