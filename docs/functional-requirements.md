...
- **FR-AGENT-10**: **`save_plan(file_path?)`**, **`export_pdf(file_path?, paper_size?, orientation?)`**, **`export_dxf(file_path?)`**, and **`export_csv(kind, file_path?)`** write a file to disk via the same services the GUI's File > Export/Save menu uses — the PDF report, a DXF drawing, or a CSV (`kind` = `'shopping_list'` or `'harvest'`), and the project's own `.ogp`. None of these mutate the in-memory plan except `save_plan`, which persists exactly what's already on screen (the same effect as Ctrl+S) — no token auth is required (reserved for D2's scene-mutating write tools). Omitting `file_path` writes to a sensible default next to the currently open project (or the app's Documents folder); `save_plan` with no project open and no `file_path` raises rather than inventing a filename, mirroring the GUI's own Save-As requirement for a brand-new project. Given an explicit `file_path`, the destination's parent directory must already exist. All four return a shared `ExportResult` (written path, format, row count for CSV, and — for `save_plan` — the project's prior file path if the call redirected it).
