...
- **FR-SUN-08**: A plant with an assigned species AND a **planting date** (the existing Plant Details field) is rendered at its **date-projected size** wherever the sun/sim timeline applies: its shadow (US-E3) and hours-of-sun contribution (US-E4) use the projected height and canopy spread, and the 3D view (US-E6) extrudes it at the projected height with the projected canopy — scrubbing the sim date forward regrows the garden consistently everywhere from one linear model (`min → max` over years-to-maturity: species `days_to_maturity` when present, else tree 10 y / others 3 y / annuals ≈ a season; seedlings start at the species minimum or 10 % of max). An explicit object-height override always wins; a plant **without** a planting date behaves exactly as before (mature size everywhere) — nothing changes for existing plans. Scrubbing NEVER mutates stored geometry (the drawn footprint stays the mature canopy; save files are byte-identical). **Stated exclusions**: linear growth only (no sigmoid/dormancy/pruning), no separate growth toggle (the sim timeline is the growth timeline), the 2D canvas keeps drawing the stored footprint — growth shows in shadows, heatmap and 3D.
