...
- **FR-CONTAINER-03**: A **trellis** is a plant-parent that holds **no soil**: `is_plant_parent_type()` (= soil containers + trellis) gates reparenting/drag-propagation/contained-plants, while the trellis is excluded from every soil feature. (Vertical 1-D plant spacing for trellises is deferred to US-C3b.)
