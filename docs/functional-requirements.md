...
- **FR-ICON-3C-04**: No emoji or unicode pseudo-icon remains in chrome: seed viability (✓ ✗) → status icons in the Viability column; task/calendar urgency dots (●) → painted themed markers; go-to arrows (→) → arrow icons; plant-search type glyphs (🌳🌿🌸🌱, also inside translated checkbox labels) → the gallery's tree/shrub/flower/seedling icons; weather refresh/expand (🔄 ▾ ▴) and the WMO condition emoji map → refresh/chevron icons and sun/partly-cloudy/fog/rain/snow/storm icons; journal photo marker (📷) → camera icon; companion "nearby" star (★) → star icon; the panel info glyph (ℹ️) and accordion chevrons (▼ ▶) → provider icons; the constraints panel's type glyphs (◯ 🔒 ↔ ↕ ⦿ ⌒ ≡ ∥ ⊾ ∠, in labels and tooltips) → the constraint toolbar's own type icons and plain words, its "×" delete button → the cross icon; the calendar's propagation "↳" sub-row marker and "↺" reset buttons → chevron/refresh icons; the succession dialog's ✓/⚠/· note prefixes → a list with status icons; the tri-state frost tooltip's ☑ ☐ ‒ → words; the 3D window's text-only Refresh/Walk toolbar → icons; the season manager's ✓ ✗ ▶ → status icons + bold; the canvas fixed-padlock badge keeps its painted body/shackle and drops the trailing 🔒 text; the theme's spin-box arrows are path SVGs instead of `<text>` glyphs. Textual arrows inside sentences ("File → Set Location", "A → B: reason", "pH 5.8 → 6.5"), canvas dimension-label prefixes and combo separator rows are prose and stay (§8.21.5 lists them; the static guard names the exceptions).
