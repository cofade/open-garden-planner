...
| **Agent write tool** | An MCP tool that mutates the live plan: `create_object`, `move_object`, `delete_object` (D2.0/D2.1), `resize_object`, `rotate_object` (D2.2), `set_species`, `set_parent_bed` (D2.3), `arrange_object` (issue #338, ADR-043), and the six layer writes (`set_object_layer`, `create_layer`, `rename_layer`, `delete_layer`, `set_active_layer`, `set_layer_property`, D2.4). Registered only when editing is enabled AND a token is set. Every document mutation applies exactly one undoable command via `command_manager.execute`; `set_active_layer` changes session state only and has no undo entry, while `move_object`'s reparent case is the only documented two-step exception. See FR-AGENT-13/14/15/16/17, ADR-036 |
