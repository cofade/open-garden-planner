...
| **Canonical geometry-apply path** | `ui/canvas/geometry_apply.py` — the one `apply_func` every `ResizeItemCommand` for a rect-backed item is given, shared by the properties panel, the drag handles and the Agent API's `resize_object`. Exists because that `Callable` parameter had produced one private copy per call site and the copies drifted, leaving a rotated object displaced after a numeric-entry resize (US-D2.2, §11.4) |
