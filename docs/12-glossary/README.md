...
| **Agent write tool** | An MCP tool that mutates the live plan: `create_object`, `move_object`, `delete_object` (D2.0/D2.1), `resize_object`, `rotate_object` (D2.2), `set_species`, `set_parent_bed` (D2.3), `arrange_object` (issue #338, ADR-043). Registered only when editing is enabled AND a token is set; each applies exactly one undoable command via `command_manager.execute` (one agent write = one Ctrl+Z step — `move_object`'s reparent case is the single documented exception). See FR-AGENT-13/14/15/16, ADR-036 |
...
| **JS view capture** | The satellite-import fallback path (issue #346): the picker positions the live Maps-JavaScript-API map over the drawn rectangle and the Python side grabs the `QWebEngineView` widget's pixels (`QWidget.grab()` — never DOM access), which are then cropped and scaled with the same analytic Web-Mercator math as the Static path and stamped with an attribution strip. Used when Google's EEA terms make the Static API reject satellite requests. Source marker: `geo_metadata.source = "google_js_view_capture"`. See ADR-019 addendum, §6.3.1 |
