...
| **Agent API** | The `agent_api/` subsystem: a default-on (read-only), loopback-only embedded MCP server (epic #237, US-D1.1) that a Preferences toggle can disable. Read-only in this phase (tool `get_plan_summary`); built write-ready. See FR-26 |
