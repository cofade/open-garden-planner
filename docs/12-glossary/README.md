...
| **Agent write tool** | An MCP tool that mutates the live plan: `create_object`, `move_object`, `delete_object` (D2.0/D2.1), `resize_object`, `rotate_object` (D2.2), `set_species`, `set_parent_bed` (D2.3), `arrange_object` (issue #338, ADR-043), the six layer writes (`set_object_layer`, `create_layer`, `rename_layer`, `delete_layer`, `set_active_layer`, `set_layer_property`, D2.4), and `undo`/`redo` (#353). Registered only when editing is enabled AND a token is set. Every document mutation applies exactly one undoable command via `command_manager.execute`; `set_active_layer` changes session state only and has no undo entry, while `move_object`'s reparent case is the only documented two-step exception. `undo`/`redo` operate the same global stack one command at a time and explicitly refuse empty stacks. See FR-AGENT-13/14/15/16/17/19, ADR-036 |
...
| **Callout offset bound** | The signed `box_dx`/`box_dy` displacement from a callout's leader tip to its text box. Agent input must be finite and satisfy `abs(offset) <= 2 * max(canvas_width_cm, canvas_height_cm)`; valid negative/default offsets remain legal. See FR-AGENT-18/20, issue #355. |
...
| **Canonical geometry-apply path** | `ui/canvas/geometry_apply.py` — the one `apply_func` every `ResizeItemCommand` for a rect-backed item is given. Exists because that `Callable` parameter had produced one private copy per call site and the copies drifted, leaving three rotated-geometry bugs in shipped GUI paths (US-D2.2, §11.4). The authoritative list of callers — and of the two deliberate exceptions — is `ui/canvas/geometry_apply.py`'s own module docstring, stated **there only**. |
