...
| **arc_to_painter_path** | Builds an arc's `QPainterPath` from exact cubic-Bézier segments (≤90° each, anchors placed analytically on the circle, control factor `k=4/3·tan(Δ/4)`). Replaces Qt's `arcMoveTo`/`arcTo`, whose rendered endpoints drift on shallow large-radius arcs (issue #195). Used by `ArcItem` and the arc-tool preview. See ADR-025 |
