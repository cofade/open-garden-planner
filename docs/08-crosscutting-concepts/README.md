...
**Worker shutdown.** `MapPickerDialog.closeEvent()` must never call `QThread.wait()` for the Static Maps worker: a single `requests.get(timeout=10)` cannot observe interruption until the response or timeout. The dialog ignores the close event, requests interruption, and performs the actual rejection asynchronously after the worker's terminal state is delivered. Unexpected exceptions are logged only after formatting and `_scrub_key()` redaction; the user-facing message remains generic. `tests/integration/test_map_picker_dialog.py::TestFetchFlow::test_close_does_not_block_on_in_flight_worker` pins the non-blocking lifecycle and `test_worker_logs_scrubbed_traceback_for_unexpected_failure` pins secret exclusion.
