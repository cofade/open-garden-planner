...
**The hide/restore itself emits `scene.changed` — asynchronously.** The hide and the restore each queue their own `changed(list[QRectF])` emission (the hide's carries the hidden items' rects, the restore's is empty), delivered on *two separate later event-loop turns* — both *after* the render method has returned. If the thumbnail refresh is (as it should be) driven by `scene.changed`, this is a self-sustaining loop: with a single overlay item on the scene the minimap re-rendered the whole plan ~9×/s forever while idle (issue #305, §11.4). A plain in-call re-entrancy flag does **not** cover it. The shipped pattern in `MinimapWidget._do_update()`: skip the toggle when nothing needs hiding; otherwise set a suppress flag before hide/render/restore and clear it with two *nested* `QTimer.singleShot(0, …)` posted *after* the restore — the queued self-inflicted emissions are already ahead of them in the queue, so `_schedule_update` sees the flag and ignores both, while any later real change still schedules a render. (A single singleShot was measured to clear the flag between the two emissions; the loop survived.)

**Rule — every slot connected to `scene.changed` (directly or via a debounce timer it restarts) must be idempotent.** Never call `update()`, `setVisible()`, `setPos()`, `setToolTip()`, `prepareGeometryChange()`, or add/remove items from such a slot unless the underlying value actually changed (early-return guard, like `GardenItemMixin.set_companion_highlight()`/`set_spacing_overlap()`/`set_capacity_overrun()`). `CanvasView._update_soil_mismatches()` violated this with an unconditional `item.update()` and looped through the 500 ms debounce with a single bed on the plan (#305). To check a change for a new loop: settle the app for 1.5 s, then assert 0 `scene.changed` emissions and 0 minimap renders over the next 2 s (`tests/integration/test_idle_scene_quiescence.py`).
