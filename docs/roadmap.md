...
| ✅     | US-E5 | 3D engine spike — **GO: PyQt6-3D** (ADR-038, owner sign-off on the PR). Version-matched 6.11.0 wheel; the one trap: `PyQt6-3D-Qt6` must pin to `PyQt6-Qt6`'s micro (DLL "Prozedur nicht gefunden" otherwise). Decisive frozen-exe gate PASSED: `--spike-3d` in the packaged exe renders lit extruded boxes from a real `.ogp` (screenshot in ADR-038), dist +13 MB (limit +150). PyVista rejected (+466 MB), pyqtgraph.opengl rejected (no directional-light control / no FPS camera). Spike stays dormant behind the flag, pinned never-imported-at-startup. §11.1 row closed | [#260](https://github.com/cofade/open-garden-planner/issues/260) | E2 |
