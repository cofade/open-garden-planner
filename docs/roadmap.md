...
| ✅     | US-E6 | 3D view MVP — View → "3D View…": real prism extrusion of the US-E3 footprints × US-E2 heights (Qt-free `core/scene3d.py`: ear-clip triangulation + triangle soup + the pinned light gate (−0.2056, −0.4675, 0.8598) + sun∥shadow agreement invariant), flat decals for height-less items, orbit camera, sun light follows the sim time/location (night → dim overhead). One Qt3D import boundary (`ui/view3d/qt3d_adapter.py`); lazy load pinned by startup test; deps `PyQt6-3D==6.11.0`+`PyQt6-3D-Qt6==6.11.0` land in pyproject per ADR-038. Viewer-only MVP: manual Refresh, lighting-only (no engine shadow maps — stated). See ADR-038 addendum, FR-SUN-06 | [#261](https://github.com/cofade/open-garden-planner/issues/261) | E5 GO |
| ✅     | US-E7 | First-person walkthrough — "Walk" toggle in the 3D View (engine `QFirstPersonCameraController`): WASD/arrows + hold-left-mouse look, fixed 165 cm eye height, plan-bounds clamp (+150 cm margin) via Qt-free `core/walk_camera.py` (pitch ±89° — no gimbal flip), Esc exits, orbit camera restored exactly (pinned). Sim-time light stays live while walking. Stated exclusions: no collision, flat ground, fixed eye height. See ADR-038 addendum, FR-SUN-07 | [#262](https://github.com/cofade/open-garden-planner/issues/262) | E6 |
| ✅     | US-E8 | Growth-over-time visualization — Qt-free `core/growth_model.py` (linear min→max over years-to-maturity: species `days_to_maturity` → else tree 10 y / 3 y / annual ≈season; issue gate table pinned). **No new metadata** — reuses the existing `plant_instance.planting_date` (verify-first paid off). Resolver `at_date` param routes dated plants through the model (explicit override wins; undated = unchanged); shadow overlay, heatmap and 3D snapshot all pass the sim date + shrink the canopy footprint to projected spread — scrub +5 y and the garden regrows everywhere from one model. Stored geometry never mutated (byte-identical-save pin, #218/#219 scars). **Two stated exclusions pending owner sign-off** (surfaced in the PR, not hidden behind the ✅): the issue's optional growth toggle (redundant — the sim timeline is the growth timeline) and the 2D canvas paint-time circle scale (growth shows in shadows/heatmap/3D; a paint-only scale is implementable as a follow-up if wanted). See ADR-037 growth addendum, FR-SUN-08 | [#263](https://github.com/cofade/open-garden-planner/issues/263) | E6 |
