...
| **15** | **v1.25.0 – v1.26.8** | **📋 Packages 1–2 + 3a shipped, 3b → 3c in progress** | **Visual Refresh** |
...
| 🔄 | 3 | **Scoped 2026-08-17** (decision log on the epic; ADR-042, FR-30) into three sequential PRs — `minor` label on whichever merges last (v1.27.0). **3a — generated object sprites** ([#308](https://github.com/cofade/open-garden-planner/issues/308)): all 15 furniture/infrastructure SVGs regenerated by `scripts/generate_object_sprites.py` in the "Lush Object" style (full parity with the plant sprites, no baked shadow, viewBox = footprint, `--check` determinism + allowlist/budget/visual-weight gates), roster grown by 9 types (sandbox, trampoline, hot tub, swing, picnic table, hammock, wheelbarrow, pergola, bird bath), aspect-preserving gallery thumbnails, square BBQ art, forward-compat pin (no FILE_VERSION bump). **3b — textures** ([#309](https://github.com/cofade/open-garden-planner/issues/309)): all 21 legacy textures → Pillow forge, Lush everywhere, numpy wrap-blur for a true byte gate, `KNOWN_SEAMED_LEGACY` emptied. **3c — iconography** ([#310](https://github.com/cofade/open-garden-planner/issues/310)): status bar, tabs + frost badge, Plants/Garden + all File/Edit/View residue, layers_panel migration, `lawn.svg`, code→file gate, all emoji pseudo-icons, View menu regrouped into submenus, Tasks/Harvest → Ctrl+4/5. | [#284](https://github.com/cofade/open-garden-planner/issues/284) | 3a: ✅ [PR #322](https://github.com/cofade/open-garden-planner/pull/322) → v1.26.8 (2026-08-18) · 3b/3c: — |
