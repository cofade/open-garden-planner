...
## ADR-023: Snap Pipeline v2 + Paper Space MVP

**Status**: Accepted (Phase 13 — Package B, US-B4/B5/B6 + US-B7)
**Context**: ADR-020 (snap registry + quadtree) shipped four kinds — endpoint, center, edge, midpoint, intersection. Package B added three more (nearest, perpendicular, tangent), two of which (perpendicular, tangent) are *not* a function of the cursor and the scene alone — they need a **second** anchor: the active tool's `last_point`. Perpendicular drops a foot from `last_point` onto the nearest edge; tangent finds the contact point of a line from `last_point` to a circle's tangent. Bolting these onto `SnapProvider.candidates(cursor, items, threshold)` would have meant either threading `view._drawing_tool.last_point` through every provider or having providers look it up themselves, breaking the pure-function contract. Separately, **B7 — Paper Space** needed a way to embed a *region* of the model canvas inside a different scene (the printable page) without lifting + duplicating items, and the rendering logic was already duplicated three times across PNG export, PDF report, and the print dialog.

**Decision**: (1) Extend `SnapProvider.candidates()` with a single new optional argument: `reference_point: QPointF | None = None`, propagated through `SnapRegistry.best()` and `PointSnapper.snap()`. Existing providers (anchor, midpoint, intersection) accept the kwarg and ignore it; new providers (`PerpendicularSnapProvider`, `TangentSnapProvider`) act on it; `NearestSnapProvider` doesn't need it. `CanvasView._active_tool_reference_point()` centralises the lookup so callers don't poke into `tool_manager._active_tool` directly. Priority ordering: endpoint 10 < intersection 15 < center 20 < perpendicular 25 < tangent 26 < midpoint 30 < edge 40 < nearest 45 — the new modes slot below the established kinds. (2) Paper Space lives in a new `ui/paper_space/` package with `PaperSpaceScene`, `PaperSpaceView`, `ViewportItem`, `TitleBlockItem`, `ScaleBarItem`, `page_sizes`. The viewport caches its render as a `QPixmap`, invalidating on `source_scene.changed`. The render path goes through a new `services/scene_rendering.render_scene_region()` helper (hide overlays / construction / scale text + render + restore via context managers); `ExportService.export_to_png` was refactored to use it as a second caller, both as deduplication and as a regression test that the helper preserves PNG export semantics.
...
- *Embed the paper-space scene inside the existing canvas tab* — too cramped UX; users need a separate workspace.
- *Live-mirror the source scene by reparenting items* — would mutate the source scene every paint and break selection / undo. The pixmap-cache approach keeps source and paper space fully isolated.
- *Render the viewport on every paint without a cache* — fine at 50 items, janky at 1000. The cache invalidates only on `source_scene.changed`, so model edits propagate but a paper-space pan / zoom is a single pixmap blit.

**Consequences**: The snap signature change is a one-time API churn — all in-tree providers were updated atomically, and external consumers (none yet) get a `None` default. Glyph rendering moved into `CanvasView.drawForeground` (each kind has its own shape: square=endpoint, circle=center, triangle=midpoint, X=intersection, hourglass=nearest, ⊥=perpendicular, small-circle+tangent-line=tangent) — no separate `snap_indicator.py` needed. Paper Space MVP scope is **one layout, one page, one viewport** per project; multi-page / multi-viewport / multiple layouts / custom title blocks are explicit out-of-scope (release-notes call-out). The shared `scene_rendering` helper now owns *all* bookkeeping at the PNG-export call site: `export_to_png` computes the font point size via `compute_export_font_size` and passes it to the helper, which handles overlay-hiding, construction-hiding, text scaling, Y-flip, and `scene.render` in one place. The legacy `_hide_*` / `_prepare_text_*` helpers on `ExportService` remain in the codebase for `export_to_svg` and `print_dialog` — those will migrate in a follow-up PR (out of B7 scope). The viewport's pixmap cache invalidates on `source_scene.changed` through a 150 ms debounce so a drag burst on the model canvas coalesces into one rebuild instead of N. **Load-time guard**: the loader rejects projects with a `version` higher than `FILE_VERSION` so an older binary can't silently round-trip a newer file (and drop unknown content on save). Tests: `test_perpendicular_snap_provider.py` (8), `test_tangent_snap_provider.py` (7), `test_nearest_snap_provider.py` (11) + integration tests; `test_scene_rendering.py` (3), `test_viewport_item.py` (5), `test_paper_layout_persistence.py` (5 — including "newer file version is rejected"), `test_export_png_after_refactor.py` (2), `test_paper_space_workflow.py` (5 — including the "save without visiting Layout tab preserves loaded layout" P0 regression test).
