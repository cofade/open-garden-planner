...
2. **One rotation-aware primitive owns "resize about a scene anchor".** `resize_handle.resize_rect_item_keeping_anchor(item, new_rect, scene_anchor, local_anchor)` applies the rect, re-pins the origin to `new_rect.center()`, and sets `pos = scene_anchor − O − R(θ)·(local_anchor − O)` (derived from Qt's `mapToScene(p) = pos + O + R(θ)·(p − O)`). The centre-anchor case (`local_anchor == O`) collapses to `set_radius_centered`'s old `pos = scene_center − O`. `set_radius_centered`, the interactive drag, and all three `apply_geometry` undo closures route through it.
3. **The drag fix is rotation-gated to stay risk-free for the common case.** `_reanchor_after_rotated_resize` runs **only when `item.rotation() != 0`**; at rotation 0 the rotation matrix is identity and the existing axis-aligned path is already correct, so it is left byte-for-byte unchanged (and every pre-existing un-rotated resize test stays green untouched).
4. **Rect-bearing items only.** The `hasattr(item, 'rect')` gate (as in #219) scopes the helper to `Circle`/`Rectangle`/`Ellipse`, which serialize `pos + rect.center()` with rotation as a separate angle. Polygons/polylines scale vertices and serialize absolute geometry, so they keep the existing path.
...
- *Rewrite the `ResizeHandle._apply_resize` → item `_apply_resize` contract to pass the fixed anchor end-to-end.* Correct but high-churn across every item type (dimension display, annotations, polygon ridge bookkeeping all live in the per-item methods). The rotation-gated post-correction achieves the same result with a far smaller, safer diff. **Deferred technical debt:** the post-correction reverse-engineers the fixed corner from `pos_dx == 0.0`/`pos_dy == 0.0`, an invariant nothing enforces against a future item type that anchors differently under clamping/squaring. Threading the fixed anchor end-to-end (with a per-item rect-normalization hook for the circle's `min(w,h)` squaring) would remove the inference, the rotation gate, and the double-apply — a worthwhile follow-up.

**Consequences**: Adds `core/plant_sizing.py`; `effective_spacing_radius()`/`paint()`/`boundingRect()`/`plant_species_assignment` delegate to it; `resize_handle.py` gains the shared primitive + the rotation-only re-anchor; the three `apply_geometry` closures re-pin the origin. No `.ogp` format change, no new dependency, no user-visible string. Tests: `tests/unit/test_plant_sizing.py` (precedence pinned behavior-preserving) and `tests/integration/test_rotation_aware_resize.py` (rotated drag-resize keeps the fixed corner + pivot; undo restores both; helper matches `set_radius_centered`). The #206 "rebuild on every repaint" overlap-refresh smell is untouched.
