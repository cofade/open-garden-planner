...
## 5.6 Agent API Subsystem (US-D1.1/D1.2/D1.3)

Black-box view of the embedded MCP server for AI agents. See ADR-033/034, FR-26, §8.19. Default-on (read-only), loopback-only, toggle to disable; read-only so far (structural/spatial/diagnostics/vision tools); built write-ready.

| Building block | Responsibility | Interface (in → out) |
|----------------|----------------|----------------------|
| `agent_api/server.py` (`AgentApiServer`, `build_server`) | Build a `FastMCP`, mount its `streamable_http_app()`, and run `uvicorn` on a daemon thread (own asyncio loop). Registers all 10 tools, each `async def` + `anyio.to_thread.run_sync(provider)`. Lifecycle: `start()` (pre-bind port → `PortInUseError`, poll `started`), `stop()` (`should_exit` + join), `is_running`, `url`. `mcp`/`uvicorn` imported lazily (`Image` is a real top-level import — see ADR-034 addendum). | `AgentProviders` + host/port → running server |
...
| `agent_api/providers.py` (`AgentProviders`) | Frozen dataclass bundling the main-thread-marshaled callables tools read from: `snapshot`, `diagnostics` (no-arg), `render` (parametrized — region/layers/width in, PNG+metadata dict out). Qt-free; the extension seam for D1.4 exports/writes. | — |
| `agent_api/schema.py` (`PlanSummary`, `ObjectRef`, `ObjectDetail`, `Diagnostic`, `Measurement`, `RenderMeta`) | Curated, stable pydantic contracts for agents, decoupled from `.ogp`/`FILE_VERSION`. Qt-free. | models |
...
| `agent_api/render.py` | The one Qt-**touching** module in the package. `resolve_image_pixel_size` (pure) clamps requested width + derived height to `[128, 2048]`. `render_canvas_image` resolves the default region, temporarily hides non-allowlisted layers, calls `services/scene_rendering.render_scene_region`, and PNG-encodes — one atomic main-thread call. | scene + region/layers/width → PNG bytes + render metadata dict |
| `core/project.py` (`ProjectManager.snapshot_dict`/`diagnostics_snapshot`) | In-memory, read-only `.ogp`-shaped dict (+ `agent_meta`) via `_build_project_data(scene, sync_journal=False)`; harvests each garden item's warning flags into Qt-free records. Reading never mutates state. | scene → dict / `list[dict]` |
| `app/application.py` (wiring) | `_setup_agent_api` (bridge + deferred auto-start), `_agent_snapshot`/`_agent_diagnostics`/`_agent_render` (one `run_on_main` hop each), `_start/_stop_agent_api`, `_on_preferences` (live restart), `closeEvent` (abort + stop). | settings + signals → server lifecycle |
