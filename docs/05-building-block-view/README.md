...
│   │   ├── geometry_apply.py     # THE canonical resize/rotate apply path — panel + handles + agent (US-D2.2)
...
| `agent_api/providers.py` (`AgentProviders`) | Frozen dataclass bundling the main-thread-marshaled callables tools use: reads `snapshot`/`diagnostics`, exports `render`/`save_plan`/`export_pdf`/`export_dxf`/`export_csv`, and writes `create_object` (D2.1/D2.5) / `move_object` / `delete_object` (D2.0) / `resize_object` / `rotate_object` (D2.2) / `set_species` / `set_parent_bed` (D2.3), the six layer providers (D2.4), and `undo`/`redo` — every document write runs one undoable command via `command_manager.execute` on the main thread; `set_active_layer` is the deliberate session-state exception and has no undo entry. `create_object` and `resize_object` are spelled as `Protocol`s rather than `Callable`s so their parameter NAMES are part of the contract (both take several `float | None` arguments a transposition would silently swap). Every field is required (no defaults), so a newly added provider cannot be silently left unwired. Qt-free. | — |
| `agent_api/schema.py` (`PlanSummary`, `ObjectRef`, `ObjectDetail`, `Diagnostic`, `Measurement`, `RenderMeta`, `ExportResult`, `WriteResult`, `HistoryResult`) | Curated, stable pydantic contracts for agents, decoupled from `.ogp`/`FILE_VERSION`. `WriteResult` (D2.0–D2.5) is the write-tool confirmation (`item_id`/`action` ∈ create|move|delete|resize|rotate|set_species|set_parent_bed|arrange|set_object_layer|create_layer|rename_layer|delete_layer|set_active_layer|set_layer_property / `undo_description` + resulting `x`/`y` where applicable, plus the shape/edit fields). `HistoryResult` describes one global GUI/agent `undo`/`redo` call and reports the post-call `can_undo`/`can_redo` state. `set_active_layer` deliberately has no undo description because it changes session state only. The `action` `Literal` values are themselves drift guards. Qt-free. | models |
...
| `agent_api/edits.py` (`validate_resize_request`, `validate_rotation`, `require_plant_parent_type`, D2.2/D2.3) | Pure validation for the edit tools, the counterpart to `creates.py`: absolute resize dimensions against the object's shape (reusing `creates`' finite/positive/sane-extent bounds, including the plant-diameter cap), rotation angles (finite, plausible, normalised into `[0, 360)`), and whether a target can parent a plant. Inlined `PLANT_PARENT_TYPE_NAMES` keeps it Qt-free (drift-guarded by `tests/unit/test_agent_api_edits.py`; note `TRELLIS` is a plant parent but not a soil container, §8.14/ADR-017). Shape *capability* is decided on the main thread by `geometry_apply.is_resizable_rect_like`, not by a name set here. | edit params → validated dimensions / angle, or `ValueError` |
...
| `ui/canvas/geometry_apply.py` (`apply_rect_like_geometry`, `apply_rotation`, `build_*_resize`, D2.2) | Qt-**touching**, main-thread only. **The one geometry-apply path for rect-backed items** — the `apply_func` every rect-backed `ResizeItemCommand` is given, plus the only `apply_rotation` definition in the codebase (drift-guarded). The authoritative list of callers — and of the two deliberate exceptions — is `ui/canvas/geometry_apply.py`'s own module docstring, stated **there only**. Enumerating it here is what this module exists to stop. Applies `prepareGeometryChange` → `setRect` → the #219 `transformOriginPoint` re-pin → `setPos`; the builders express the two anchor policies (`keep_center=True|False`) and solve `pos` through `resize_handle.anchored_position`. Extracting it fixed a live panel bug the copies had drifted into (§11.4). | item + target extents → `(old, new)` geometry dicts / applied geometry |
| `app/application.py` (wiring) | `_setup_agent_api` (bridge + deferred auto-start), `_agent_snapshot`/`_agent_diagnostics`/`_agent_render`/`_agent_save_plan`/`_agent_export_pdf`/`_agent_export_dxf`/`_agent_export_csv` (one `run_on_main` hop each), all D2 write bodies including `_agent_undo`/`_agent_redo`, duplicate-ID fail-closed deletion and post-delete absence checks, `_start/_stop_agent_api`, `_on_preferences` (live restart), `closeEvent` (abort + stop), `agent_api_running_url` (D1.6, the single accessor both the Help menu and Preferences dialog query — never a settings/widget-state reconstruction), `_on_connect_ai_assistant` (D1.6, opens `ConnectAiAssistantDialog` with that URL). | settings + signals → server lifecycle |
