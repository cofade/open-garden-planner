...
| Building block | Responsibility | Interface (in → out) |
|----------------|----------------|----------------------|
| `agent_api/server.py` (`AgentApiServer`, `build_server`) | Build a `FastMCP`, mount its `streamable_http_app()`, and run `uvicorn` on a daemon thread (own asyncio loop). Registers all 14 tools, each `async def` + `anyio.to_thread.run_sync(provider)`, plus 5 `@mcp.resource()` (`garden://plan`, `garden://plan/raw`, `garden://canvas.png`, `garden://diagnostics`, `garden://species`) and 2 `@mcp.prompt()` (`audit-plan`, `describe-garden`) registrations (D1.5) reusing the same providers/mapping/diagnostics functions as the tools. Lifecycle: `start()` (pre-bind port → `PortInUseError`, poll `started`), `stop()` (`should_exit` + join), `is_running`, `url`. `mcp`/`uvicorn` imported lazily (`Image` is a real top-level import — see ADR-034 addendum). | `AgentProviders` + host/port → running server |
...
| `agent_api/providers.py` (`AgentProviders`) | Frozen dataclass bundling the main-thread-marshaled callables tools read from: `snapshot`, `diagnostics` (no-arg), `render`/`save_plan`/`export_pdf`/`export_dxf`/`export_csv` (parametrized). Qt-free; the extension seam for D2 writes. | — |
| `agent_api/schema.py` (`PlanSummary`, `ObjectRef`, `ObjectDetail`, `Diagnostic`, `Measurement`, `RenderMeta`, `ExportResult`) | Curated, stable pydantic contracts for agents, decoupled from `.ogp`/`FILE_VERSION`. Qt-free. | models |
...
| `ui/dialogs/connect_ai_assistant_dialog.py` (`ConnectAiAssistantDialog`, D1.6) | Thin Qt shell: shows the connect URL + Copy button, one row per detected client with an "Add to …" button (where automatic install exists) and an always-available manual-snippet disclosure. Reached from Help → "Connect AI Assistant…" (live running-server URL) and a Preferences → Agent API "Connect…" button (live widget-state preview). | server URL → registered client config / clipboard |
| `app/application.py` (wiring) | `_setup_agent_api` (bridge + deferred auto-start), `_agent_snapshot`/`_agent_diagnostics`/`_agent_render`/`_agent_save_plan`/`_agent_export_pdf`/`_agent_export_dxf`/`_agent_export_csv` (one `run_on_main` hop each), `_start/_stop_agent_api`, `_on_preferences` (live restart), `closeEvent` (abort + stop), `_on_connect_ai_assistant` (D1.6, opens `ConnectAiAssistantDialog` with the running server's URL). | settings + signals → server lifecycle |
