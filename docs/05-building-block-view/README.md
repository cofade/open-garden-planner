...
## 5.6 Agent API Subsystem (US-D1.1/D1.2/D1.3/D1.4/D1.5)

Black-box view of the embedded MCP server for AI agents. See ADR-033/034, FR-26, §8.19. Default-on, loopback-only, toggle to disable; structural/spatial/diagnostics/vision reads plus four file-producing export/save tools (D1.4, no scene mutation except `save_plan`); five read-only resources + two read-analysis prompts (D1.5, no new business logic — reuse the same providers as the tools); built write-ready for D2's scene-mutating tools.
...
| `app/application.py` (wiring) | `_setup_agent_api` (bridge + deferred auto-start), `_agent_snapshot`/`_agent_diagnostics`/`_agent_render`/`_agent_save_plan`/`_agent_export_pdf`/`_agent_export_dxf`/`_agent_export_csv` (one `run_on_main` hop each), `_start/_stop_agent_api`, `_on_preferences` (live restart), `closeEvent` (abort + stop). | settings + signals → server lifecycle |
