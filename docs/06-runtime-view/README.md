...
While the Static Maps request is in flight, closing the picker requests worker interruption and leaves the dialog alive until the worker's terminal signals have been processed; the GUI thread never joins a network-bound worker. Unexpected worker failures stay generic in the dialog, while a traceback scrubbed of the resolved key is written to the module logger for diagnosis.
