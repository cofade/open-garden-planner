...
When the Static worker fails with a classifiable EEA 403 (issue #346), the dialog offers the **JS-API view capture** path; a “Capture view” button also makes it available at any time with a drawn rectangle. Capture is a GUI-thread choreography with no worker: Python picks the best integer zoom (`pick_capture_zoom`, 0.85 viewport margin) and asks the page to reposition (`window.beginCapture` → hide toolbar/hint, `disableDefaultUI`, `setCenter`+`setZoom`, wait for `idle` + `tilesloaded`, with a 15 s JS timeout); the page reports readiness through the bridge (`captureReady(token, z, dpr, cssW, cssH)`, token first — the echo of the capture generation) and Python grabs the `QWebEngineView` widget — metadata crosses the bridge, pixels never do. The grab is checked for blankness (luminance stddev), cropped to the bbox through the same `crop_image_to_bbox` seam the Static path uses, stamped with the attribution strip (`bake_attribution`), and handed to the application as a standard `FetchResult` with `source="google_js_view_capture"` and `attribution`; `BackgroundImageItem.from_fetch_result` persists both as additive `geo_metadata` keys. A 20 s Python watchdog and the generation counter guard against a lost page/bridge.
