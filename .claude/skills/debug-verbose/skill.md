...
## Case study: editing "Current Spread" shrinks the shadow but not the plant icon (#298 follow-up, fixed 2026-08-08)
...
**Root cause**: not a bug — a deliberate US-E8 scoping decision. The growth model (`core/growth_model.py`) was wired into the shadow (US-E3), heatmap (US-E4), and 3D view (US-E6) but explicitly, by design, never into the 2D canvas icon's own rendering, specifically so a young sapling's spacing footprint doesn't shrink and let the user plant too densely.

**Resolution**: since the user's complaint was about the *visual icon*, not the *spacing footprint*, and `plant_sizing.py` already establishes "multiple legitimate sizes on one item" as an accepted pattern, the fix decouples a **fourth** size — the rendered pixmap's diameter — from the footprint, using `AskUserQuestion` to confirm the trade-off first rather than unilaterally overriding a documented design decision. `CircleItem._visual_plant_diameter_cm()` calls `growth_model.grown_spread_cm(species, metadata, date.today(), object_type_name)` and falls back to the footprint diameter unchanged when no growth data is set (behaviour-preserving for the common case). `boundingRect()` was also updated: `overflow = max(0.0, (visual_diameter * SCALE - rect.width()) / 2.0)` — clamped at 0 so a *shrunk* icon never shrinks the advertised bounding rect below the footprint's own (hit-testing/selection on the mature footprint must stay intact), but still *grows* it for the edge case where a recorded current size exceeds the species max (else that oversized pixmap paints outside its advertised rect — the exact "ghost" failure mode #218 already named). `plant_database_panel._update_instance_metadata` now also calls `prepareGeometryChange()` before writing `current_spread_cm`, for that same edge case.

**Lesson**: before instrumenting a "why doesn't X update" bug, check whether the gap is *documented* — a module docstring that says "deliberately does NOT own this" is a design decision, not a bug report waiting to be filed. When a user's mental model conflicts with a documented decision, the fix is a product conversation (here, via `AskUserQuestion`) before it's a code change — the same investigation can lead to "this is correct, leave it" or "here's the fix" depending on what the user actually wants, and guessing wrong risks silently reverting reasoning a prior contributor wrote down for a reason. Once a decision is confirmed, look for the narrowest fix that doesn't undo the *original* reasoning: the spacing footprint still deliberately stays mature-sized (no change), only a fourth, purely-visual size was added alongside it.
