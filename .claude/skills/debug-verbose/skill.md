...
**Fix**: an autouse `_isolate_ui_state` fixture pointing `ui_state.QSettings` at the test key (local runs now also match CI, where the store is always pristine). Follow-up #285 tracks the real repair: route `UiStateStore` through the `app/settings.py` chokepoint so one isolation point covers everything and the fixture can be deleted.
