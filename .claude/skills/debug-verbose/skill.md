...
**Fix**: first an autouse `_isolate_ui_state` fixture pointing `ui_state.QSettings` at the test key (local runs then also matched CI, where the store is always pristine). **#285 / ADR-041 then did the real repair and deleted that fixture**: `app/settings.create_qsettings()` is now the only place in `src/` that constructs or imports a settings store, and `conftest.isolate_qsettings` rebinds `settings.ORGANIZATION_NAME` / `APPLICATION_NAME` — which the factory re-reads on every call — so one seam covers every consumer. Enforced by `tests/unit/test_settings_chokepoint.py` (an AST walk over `src/` *and* `tests/`: nobody else may name `QSettings`, and nobody may build a store at import time — such a store predates every fixture) + `tests/integration/test_settings_isolation.py` (spies `QSettings.value`/`setValue` during a full app boot).
