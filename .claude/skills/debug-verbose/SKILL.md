...
**Root cause.** Unparented `QTimer` in a `QDialog` + a timer slot that can open a modal `QMessageBox(self)`. Latent since the dialog was written; timing-flaky by nature.
