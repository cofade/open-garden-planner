...
5. **Documentation accuracy and the mandatory-update table.** Where the change touches behaviour described in docs (`CLAUDE.md`, arc42 chapters under `docs/`, ADRs in `docs/09-architecture-decisions/`, `docs/functional-requirements.md`, `docs/12-glossary/README.md`, the wiki at `../open-garden-planner.wiki/`), do the docs still match? Per CLAUDE.md's "Contributing to Documentation" table, specific doc updates are mandatory based on change type. Documentation drift is debt that compounds; flag it.
