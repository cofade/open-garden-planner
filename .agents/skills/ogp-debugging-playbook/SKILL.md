...
| 29 | Full `pytest tests/` run stops producing output for minutes; the last line is a full-app integration test (e.g. `test_trellis.py`) that passes alone | A hung full-app test — sibling of rows 14–16 (leaked slot/thread from an earlier `GardenPlannerApp` blocking a later one). Before 2026-08-17 nothing bounded it: "still running" was indistinguishable from "hung" | Watch the log's **age**, not its tail (`stat -c %Y` vs `date +%s`); kill, run the stalled file alone (green?), then after its collection-order predecessors | `pytest-timeout` is armed globally (`timeout = 180` in `pyproject.toml`) — a hang now fails loudly (Linux: just that test; Windows: stack dump + abort). Never raise the global; mark a genuinely slow test `@pytest.mark.timeout(N)` with a reason | §11.4 (2026-08-17), §8.10 |
