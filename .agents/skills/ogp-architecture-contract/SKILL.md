...
| 3 | **Exactly two ways onto the undo stack:** `CommandManager.execute(cmd)` (runs it) and `CommandManager.register_applied(cmd)` (for changes already applied live, e.g. a finished drag). Both emit `stack_changed` → `mark_dirty`. **Never** hand-append `_undo_stack` or hand-emit signals (#209; `core/commands.py:71` `execute`, `core/commands.py:89` `register_applied`). `undo()`/`redo()` also emit `stack_changed`; `clear()` does not (so new/load stays clean). | Dirtiness and every panel-refresh path hang off `stack_changed`; a third path silently skips them. | Post-save undo/close silently discards changes (the #209 data-loss bug); panels go stale. | Docstring contract in `commands.py`; `tests/integration/test_panel_refresh_wiring.py`; senior review treats a raw append as P0. |
...
| 12 | **Agent API safety model (ADR-033/034, §8.19):** (a) tool handlers run on the uvicorn thread and touch Qt **only** via `MainThreadBridge.run_on_main` (queued signal + Future); (b) Qt-touching handlers are `async def` and offload the blocking hop via `anyio.to_thread.run_sync` (a sync handler blocks the event loop — verified against mcp 1.28.1); (c) reads use `ProjectManager.snapshot_dict()` which never mutates state (`sync_journal=False`); (d) server binds **127.0.0.1 only** ("never 0.0.0.0"); (e) **reads are unauthenticated** (loopback trust) but **every scene-mutating write tool is double-gated** (D2.0, v1.24.3): the tool is *registered at all* only when `agent_api_writes_enabled` AND `agent_api_token` are both set (`writes_active` in `build_server`), and each handler calls `_require_write_auth` — constant-time compare, `isascii()`-guarded — **in the same request task, before** the `anyio.to_thread.run_sync` hop. The token arrives via `?token=` URL param (preferred — Codex drops configured headers on tool calls) or `Authorization: Bearer`; the gate is **per-tool, not blanket middleware**, so read-only D1 clients keep working; (f) shutdown calls `bridge.abort_pending()` *before* `server.stop()`. | Qt is main-thread-only; an unauthenticated localhost server must not be able to mutate the user's plan; close must not deadlock on an in-flight hop. | UI freezes/crashes from off-thread Qt access; any local process could edit the plan; hang on app close. | `test_agent_api_bridge.py` (marshaling/timeout/abort), `test_agent_api_server.py` (end-to-end), the `bridge.py` house-rule docstring. |
| 13 | **Agent writes go through the command system** — one agent operation = **one undoable command** on the same `CommandManager` (invariants 3–4 unchanged; move/set-position reparenting is the documented two-step exception); never a parallel mutation path. D2.0 set the precedent that a write mirrors the **GUI's whole orchestration**: `move_object` carries bed children and reconciles membership, while `delete_object` strips constraints and a HOUSE ridge. D2.6 reuses that move helper for absolute placement and routes interactive and agent vertex mutations through the shared builders in `ui/canvas/geometry_apply.py`. All geometry-changing tools **refuse** constrained items at `_agent_require_unconstrained`; `_resolve_agent_item` additionally refuses journal pins, locked-layer items, and individual group members. | The user must Ctrl+Z the whole semantic operation; private callback/mutation copies drift, and a partial solver run silently violates relationships or constraints. | The D2.0 review P0: a bare `MoveItemsCommand([item], delta)` abandoned a bed's plants; D2.6's alternative — returning raw pre-rotation points or silently moving one constrained item — would create the same class of lie. | ADR-033/036 D2.0–D2.6; `test_agent_api_writes.py`; `test_agent_api_geometry.py`; `test_agent_api_default_on.py`. |
...
| 19 | **Stacking order (issue #338, ADR-043):** a sparse rank `GardenItemMixin.stack_order` is written **only** by `CanvasScene.addItem` (unranked item → top of its layer), `MoveToLayerCommand` (only items whose layer actually changes), `ArrangeItemsCommand` (renumbers the affected layers), and the post-load renumber when `ProjectManager.load`'s `CanvasScene.suspend_z_refresh(renumber=True)` scope exits. The z-value refresh (`_refresh_layer_z`/`_update_items_z_order`) is a pure derivation from whatever ranks already exist — it **never** writes a rank. There is exactly **one apply seam**, `ui/canvas/arrange.py::build_arrange_command`, shared by the Edit ▸ Arrange menu, every item's `Arrange ▸` context-menu submenu, the Properties panel's Arrange row, and the agent's `arrange_object` tool. `stack_order` is an additive `.ogp` key — no `FILE_VERSION` bump. | A refresh that also renumbers ranks would silently invalidate a command's undo snapshot taken before that refresh happened to run (caught in #338's design review, not shipped) — the four-writer discipline is what keeps undo/redo exact; one apply seam is the same lesson §8.19/`geometry_apply.py` already teaches for resize/rotate. | A copy-per-call-site "arrange" implementation drifts like the pre-D2.2 resize/rotate closures did; a rank-writing refresh breaks Ctrl+Z for any command that happens to run between an unrelated refresh and its own undo. | ADR-043; §8.25; `tests/unit/test_stacking.py`; `tests/integration/test_stacking_roundtrip.py`; `tests/integration/test_arrange_z_order.py`. |
...
- **D2 agent writes inherit all of this** (invariant 13): one MCP write call →
  one `run_on_main` hop → one command via `execute()`/`register_applied()` (the
  documented move/set-position reparent case is the sole two-step exception).
  D2.6 vertex writes use the same canonical builders in
  `ui/canvas/geometry_apply.py` as the interactive polygon/polyline paths; never
  add a private callback in `application.py`.
...
| **Agent API reads are unauthenticated.** Loopback-only, on by default — any local process can read the open plan. **Writes are token-gated** since D2.0 (see invariant 12e), but the token travels in the connect **URL**, so it is visible in local request logs (accepted tradeoff, ADR-036). | Reads open by design; writes gated |
...
| **Smart-symbol `version` field is a strict geometry contract:** editing a bundled symbol's geometry without bumping its `version` silently re-renders already-saved plans (ADR-032). Sidebar panel currently hidden via one line: `application.py:3689` `set_panel_visible("smart_symbols", False)`. | Sharp edge + deferred UI |
...
4. **New persisted data?** Default to an additive key with old-app degrade
   ...
   alternative pattern.
5. **New ADR needed?** AGENTS.md triggers: new dependency, choosing between
