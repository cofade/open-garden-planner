...
| **Smart-symbol `version` field is a strict geometry contract:** editing a bundled symbol's geometry without bumping its `version` silently re-renders already-saved plans (ADR-032). Sidebar panel currently hidden via one line: `application.py:3409` `set_panel_visible("smart_symbols", False)`. | Sharp edge + deferred UI |
