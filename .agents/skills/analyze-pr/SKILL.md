...
Commands come from `AGENTS.md` Quick Reference. Run the four fast layers in parallel as **background** tasks (`run_in_background: true`), then the PyInstaller smoke test once they finish:
...
6. **AGENTS.md gate reminders** — only mention if the PR description does not already show evidence:
   - Senior-reviewer pass (`subagent_type: senior-reviewer` against the branch in a fresh worktree)
   - arc42 doc updates per `AGENTS.md` "Contributing to Documentation" table
